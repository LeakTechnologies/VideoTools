...
timeline
    ...
    v0.1.1-dev66 (Current) : **Settings keyboard navigation** (PageUp/PageDown/Home/End scroll the active tab) : **updater error-message hardening** (rate-limit/release-not-published/network causes surfaced in the dialog)
...
## Current State (v0.1.1-dev66)

- **Dev66 = settings keyboard navigation + updater messaging**: (1) **Settings keyboard navigation** — PageUp/PageDown scroll the active Settings tab by one viewport, Home/End jump to the top/bottom. Each tab stays a `ui.NewFastVScroll`; a new `settings.Options.ActiveScroll` resolver captures the visible tab (via `tabs.OnSelected`), and `desktop.CustomShortcut` canvas shortcuts — registered in `showSettingsView`, removed on the way out via `OnBack` — drive `ScrollBy` (480px viewport fallback; ±1e6 clamped for Home/End). (2) **Updater error messaging** — `describeUpdateError` maps connection errors, GitHub 403 rate-limits, 404 not-yet-published releases, missing platform assets and JSON failures to actionable dialog text ("Update check failed: …"); raw errors still log under `CatSystem`. The stale "PATCHed on every nightly run" comment in `fetchUpdateInfo` is corrected.
...
- **Tester verification of dev66 release** — settings keyboard navigation (PageUp/PageDown page the active tab, Home/End to top/bottom, stops after leaving Settings), updater failure dialog shows a real cause, AND everything from dev65 content (rip refinement: layout pass + LOAD DISC) + the still-pending dev64 release content (rip modes, per-language subtitle checkboxes, scan-as-you-go snippets, Convert SMPTE idle restore) + dev63/dev62/dev61 content; move roadmap cards `rip-refinement`/`rip-overhaul` `done` → `shipped` on sign-off
