...
- **Dev75 = dynamic rip-mode selection + bulk-selection deadlock fix** (in progress, tag `v0.1.1-dev75`): switching the rip-mode radio now *reshapes* the title selection to the mode on a mode transition — "Movie + extras (choose titles)" re-selects every title automatically (no manual Select All needed afterwards, which previously re-selected/blanked and froze the UI), "Main feature only" keeps only the longest title (anchored), "Scene segments only" selects just the detected segments. Manual per-title toggles inside a mode persist; only a transition re-shapes. Root-cause crash fix: Select All / Deselect All called `SetChecked` on each card while holding the ContentBrowser mutex, so the card's OnChanged handler re-acquired the same mutex and deadlocked the UI thread — a hard freeze whenever the bulk checkboxes actually changed a value. Bulk selections now raise the card's updating guard (OnChanged early-returns), `ApplyModeLock` is a purely visual layer (locked/anchored maps only), and selection is owned by `ReshapeSelection` (whole-map replacement, no per-title OnChanged). Mode → selection/lock logic is centralised in `internal/app/modules/rip/ripmode.go` (`CanonicalSelection`/`CanonicalLock`, unit-tested). A freshly loaded disc always starts in "Main feature only" and re-shapes once.
