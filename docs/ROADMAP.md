...
## Current State (v0.1.1-dev70)

- **Dev70 = rip UI polish** (released as `v0.1.1-dev70`): (1) the mode radio now reads "Movie + extras (choose titles)" and "Main feature only" (the scenes option is really per-title selection with every title checked by default, i.e. movie + extras) and stacks vertically so the longer labels fit the narrow left column; (2) the Content Browser title-card info line no longer repeats the header's T##/duration and no longer word-wraps "1 audio" onto its own line — it is a single compact, localized "N chapters · N audio · N subs" line with singular forms (new `RipTitleAudioOne/Many` + `RipTitleSubsOne/Many` i18n keys; `RipTitleCardFmt` dropped its T##/duration prefix, the header owns the duration display); (3) the subtitle-language picker gains **Select All / Deselect All** buttons (only when a title has multiple languages) that bulk-apply the whole list — selection slice is set outright and per-check callbacks are suppressed/restored so config persistence and rip mappings stay consistent.
...
- **Tester verification of dev70 release** — (1) mode radio reads "Movie + extras (choose titles)" / "Main feature only", stacks vertically, still defaults to "Main feature only"; (2) the Content Browser title-card info line is a single compact "N chapters · N audio · N subs" line with no "1 audio" wrapped onto its own line and no duplicated T##/duration; (3) the subtitle-language picker shows Select All / Deselect All when a title has multiple languages, both persist correctly, and a rip maps exactly the chosen streams
- **Tester verification of dev69 release** — re-run the `06 (1998)` Extra Title 04 rip on the Achtzehneinhalb (Bose Mosen) disc: the log shows "VOB concat: capping output at N s" and the output file's duration is the real ~30m23s, not 26hrs (Windows Explorer/player shows the title length; chapters end at the title duration); plus all dev68 content (lying-IFO Main-Feature rip via the clamp) and the still-pending dev67/dev66/dev65/dev64/dev63/dev62/dev61 content
