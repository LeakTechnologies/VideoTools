...
- **Dev76 in progress (tag `v0.1.1-dev76`)**: (1) **ISO 9660 resolve fix** — the rip resolver demanded a nested `tempDir\VIDEO_TS` folder but the native UDF/ISO 9660 readers extract FLAT (files land in the destination root), so every ISO 9660-only grey-market image failed the rip path with "`VIDEO_TS not found in ISO 9660 image`" right after a successful scan (`Lewd Conduct 1.iso` + `Barely Legal #11.iso`). `extractedVideoTSPath` accepts nested or flat (marker `targetDir.IFO` / `index.bdmv`); `VT_REAL_ISO` test passes on both real images. (2) **Menu dedup** — repeated title rips of one disc no longer write a fresh identical `_Menu_<label>.mkv` per output base; same (label, size) menus already present in the output dir are skipped ("already exported (same menu content)"), keeping the animated-menu reauthoring capability without the duplicate sprawl. (3) **Choose-titles mode starts empty** — dev75 reshaped "Movie + extras (choose titles)" to *select everything*, so a user switching modes to pick two extras got the whole disc queued ("made the title, then `_03/_04/_05`… downloading more than we want"). `CanonicalSelection` now returns empty for `""` — what is ticked is exactly what rips (Select All still opts into all; `"full"` keeps its single-job path); `"main"`/`"segments"` defaults unchanged.
...
## Now (dev75 — open, plus backlog)

- **Tester verification of dev75 release** — load any multi-title disc: switching the rip-mode radio to "Movie + extras (choose titles)" immediately re-selects every title (no manual Select All; the old flow re-selected/blanked and froze the UI), switching to "Main feature only" leaves only the main title selected+anchored and greys the rest, "Scene segments only" (on a scene-segmented disc) greys the whole-movie titles and pre-selects the segments; Select All / Deselect All work without freezing even right after a mode switch; manual per-title toggles inside a mode persist until the next mode change; a freshly loaded disc always starts in "Main feature only"
