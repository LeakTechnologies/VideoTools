...
## Current State (v0.1.1-dev65)

- **Dev65 = rip refinement follow-up (released, pending tester verification)**: (1) **Vertical-space layout pass** — Rip Log demoted to a collapsed-by-default strip (VSplit 0.97, only the LOG pill control visible; bounded ~28% expansion at 0.72; auto-opens on rip activity/errors via `SetRipLogExpand`), Disc Menu preview collapses to a ~40px strip until a menu frame loads (Preserve/Main check row hidden; Clear ISO resets it), so the Content Browser (title list) regains the flexible height without row/typography changes or touching the 55/45 HSplit. (2) **Load Disc from an optical drive** — LOAD DISC button in the rip Source row → `OnLoadDisc` → `detectOpticalDrives()` (reused from burn) → radio drive picker when several → `resolveOpticalDriveVIDEOTS` (Windows drive letter / Linux `/proc/mounts`) → normal load/scan/rip path; errors via `RipErrNoDrive`/`RipErrNoDVD`/`RipDriveNotMounted`; new i18n keys en/fr/iu/iu_latin; design at `docs/RIP_LOAD_DISC.md`.
- **Dev64 shipped** (published 2026-09-06): rip modes (scenes / full movie) + per-language subtitle selection + scan-as-you-go snippets + Convert SMPTE-idle restore. Tester verification of the *release content* still in progress.
...
- **Tester verification of dev65 release** — rip refinement: layout (Content Browser shows 4+ titles without scrolling at ~1600×900; log collapsed to a LOG pill with bounded ~28% expansion + auto-open on rip/error; menu preview ~40px strip until a frame loads) and LOAD DISC (drive picker when several drives; DVD in drive → titles load; data disc / empty drive / no drive → clean error); ALSO still pending: dev64 release content (rip modes, per-language subtitle checkboxes, scan-as-you-go snippets, Convert SMPTE idle restore) + the dev63/dev62/dev61 content; move roadmap cards `rip-refinement`/`rip-overhaul` `done` → `shipped` on sign-off
