...
timeline
    ...
    v0.1.1-dev73 (Current) : **Per-title PGC duration scan consistency** — multi-PGC VTS discs (scene-segmented titles) report each title's real duration/chapter count instead of reusing the first title-domain PGC for every title; same per-title PGC read threaded into rips for chapters/cap; Red Hairy Teens (2008) 7-title disc verified
