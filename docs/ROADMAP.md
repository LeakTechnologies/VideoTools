...
timeline
    ...
    v0.1.1-dev64 (Current) : **Rip modes + per-language subtitle selection** — mode radio (Selected scenes / Full movie of the longest title) : one subtitle checkbox per main-title language mapped to per-stream -map 0:s:<idx> : scan-as-you-go disc snippets in the DiscSummary : **Convert SMPTE-idle restore** (player reset on menu-driven re-entry) : pending tester verification
