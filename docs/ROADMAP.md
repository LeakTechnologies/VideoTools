...
timeline
    ...
    v0.1.1-dev67 (Current) : **Rip de-clutter** (Disc Menu preview + in-app Rip Log removed; Content Browser owns the column) : **rip default mode** ("Full movie (main feature)" first + default) : **settings keyboard-nav fix** (focused key-catcher widget + canvas OnKeyDown fallback — dev66's CustomShortcut never fired for unmodified keys)
...
## Current State (v0.1.1-dev67)

- **Dev67 = tester-requested rip de-clutter + settings keyboard-nav fix** (released as `v0.1.1-dev67`): (1) **Rip default mode** — the mode radio lists and defaults to "Full movie (main feature)" (was "Selected scenes"). (2) **Rip de-clutter** — the Disc Menu preview panel and the in-app Rip Log strip are removed outright so the Content Browser owns the full left-column height; rip activity/errors stay in the on-disk executor log (`menu_preview.go` deleted; obsolete `RipLog*`/`RipMenuPreview`/`RipPreserveMenus`/`RipLoadingMenu`/`RipNoMenuPlaceholder` i18n keys retired). (3) **Settings keyboard nav fix** — the dev66 `desktop.CustomShortcut` canvas shortcuts were dead: the Fyne GLFW driver only synthesizes `CustomShortcut` when the modifier is non-zero (`internal/driver/glfw/window.go`), so bare PageUp/PageDown/Home/End never fired. Now a focused 1×1 key-catcher widget (`settingsKeyNav`, `fyne.Focusable` + `desktop.Keyable`) serves `TypedKey` for every press including repeats, with canvas `SetOnKeyDown`/`SetOnKeyUp` as the fallback when nothing is focused; keys stop after leaving Settings.
...
- **Tester verification of dev67 release** — rip mode radio defaults to and lists first "Full movie (main feature)"; Settings PageUp/PageDown/Home/End actually page the active tab (focused key-catcher; every tab; keys stop after leaving Settings); no Disc Menu panel / no in-app Rip Log — the Content Browser owns the full left-column height at 1600×900 (4+ titles without scrolling); AND the dev66 content (update-check failure dialog explains the cause) + dev65 content (rip refinement: layout pass + LOAD DISC physical DVD) + the still-pending dev64 release content (rip modes, per-language subtitle checkboxes, scan-as-you-go snippets, Convert SMPTE idle restore) + dev63/dev62/dev61 content; move roadmap cards `rip-overhaul`/`rip-refinement`/`rip-refine-followup` `done` → `shipped` on sign-off
