...
timeline
    ...
    v0.1.1-dev64 (Current) : **Rip modes + per-language subtitle selection** — mode radio (Selected scenes / Full movie of the longest title) : one subtitle checkbox per main-title language mapped to per-stream -map 0:s:<idx> : scan-as-you-go disc snippets in the DiscSummary : **Convert SMPTE-idle restore** (player reset on menu-driven re-entry) : **post-release rip refinement** (vertical-space layout pass — log demoted to a collapsed strip, menu preview compact until a frame loads — + Load Disc from optical drive), committed pending tester verification : pending tester verification
...
## Current State (v0.1.1-dev64)

- **Dev64 = rip modes + per-language subtitle selection + Convert SMPTE-idle restore**: (1) **Rip mode selection (scenes / full movie)** — a horizontal radio under the subtitle checkbox switches between per-title "Selected scenes" rips and a single "Full movie (main feature)" rip of the longest title (one job through the existing per-title executor path with `vtsNumber`/`titleNumber` + `extractMode "main"`; falls back to executor defaults with no scan result); hidden while region conversion forces full-disc extraction; CTA line shows "Ready to rip main feature: Title N · duration". (2) **Per-language subtitle selection** — the single "Include subtitles" checkbox becomes one checkbox per subtitle language on the main title; `RipArgs.SubtitleSel []int` maps each selected language to its `-map 0:s:<srcIdx>` stream (blanket `0:s?` fallback for legacy callers), only selected languages are muxed, VOBSUB pairs stay together, legacy configs default to all. (3) **Scan-as-you-go snippets** — `ScanDisc`/`scanISOViaUDF`/`runISOScan` gained an `onNote` callback surfacing disc facts incrementally into the DiscSummary scanning state via `SetSnippet`. (4) **Convert SMPTE-idle restore** — `InlineVideoPlayer.CurrentPath()` + `Close()` clearing it; `showConvertView` resets the shared primary player on menu-driven re-entry unless it already shows the convert source. (5) **Post-release rip refinement (committed, pending tester verification)** — vertical-space layout pass (Rip Log demoted to a collapsed-by-default VSplit strip with `SetRipLogExpand` auto-open; Disc Menu preview collapses to ~40px until a menu frame loads; Content Browser regains the flexible height) + Load Disc from an optical drive (`OnLoadDisc` → `detectOpticalDrives` + radio picker → `VIDEO_TS` resolution; see `docs/RIP_LOAD_DISC.md`; committed as `a93fae39`).
...
- **Tester verification of dev64 release** — rip mode selection (scenes / full movie), per-language subtitle checkboxes (deselect a language → stripped from output; legacy configs default to all), scan-as-you-go snippets in the DiscSummary, Convert SMPTE idle bars restored on menu-driven re-entry (e.g. Inspect → menu → Convert) + all dev63/dev62/dev61 content; move roadmap cards `rip-refinement`/`rip-overhaul` `done` → `shipped` on sign-off
...
- **Tester verification of the post-release rip refinement** — Content Browser shows 4+ titles without scrolling (log collapsed to a LOG pill, bounded ~28% when expanded, auto-open on rip/error; menu preview ~40px strip until a frame loads) + LOAD DISC flow (drive picker when several drives; DVD in drive → titles load; data disc / empty drive → clean error); then cut dev65
- **Tester verification of in-app updater** — a dev61 binary should offer and install dev64 from Settings (dev61→dev62 was the first exercise; dev64 re-exercises from a released dev63 binary)
