...
timeline
    ...
    v0.1.1-dev63 (Current) : **dev62 follow-up patches** — Convert/Filters/Upscale metadata-fold fix (tappable METADATA header stays visible, only the body hides) : rip ISO-load scanning-state fix (SetScanning no longer clobbered by rebuildEnrich's SetEmpty) : ISO scan hardening (runISOScan recover→SetError, CatDVD logging, re-scan on module re-entry)
