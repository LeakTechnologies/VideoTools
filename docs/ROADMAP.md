...
timeline
    ...
    v0.1.1-dev69 (Current) : **VOB-concat stale-PTS duration cap** — grey-market discs whose VOBs carry a phantom tail packet at a huge PTS offset inflated fallback rips to "26hrs"; the executor now probes per-VOB durations and caps the output with -t ceil(Σ + 60s) so stray tail packets never land in the MKV
