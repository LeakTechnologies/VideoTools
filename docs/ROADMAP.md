...
timeline
    ...
    v0.1.1-dev74 (Current) : **Scene-segment rip modes + cell-accurate VOB concat** — rip offers a "Scene segments only" mode on scene-segmented discs (longest-run/segment auto-detection, whole-movie titles greyed/locked, segments pre-selected and toggleable) : the VOB-concat fallback now slices VOBs to the selected title's PGC cell byte ranges (correct content for shared-VTS extra titles, not the movie's opening) : loading a new disc no longer leaks the previous movie's Title into the output filename
