// Package css implements DVD Content Scramble System (CSS) decryption.
//
// The cipher algorithm and lookup tables are derived from publicly available
// CSS cryptanalysis research (Frank Stevenson, 1999) and the libdvdcss library.
// libdvdcss is distributed under the GNU General Public License, version 2 or
// any later version (GPL-2.0-or-later). VideoTools is GPL-3.0, which is
// compatible with GPL-2.0-or-later.
//
// CSS was reverse-engineered; no proprietary material is used here.
package css

// cssTab1 is the primary substitution table used in sector decryption and key
// decryption.
var cssTab1 = [256]byte{
	,
}

// cssTab2 is a bit-swap permutation table (256 entries).
var cssTab2 = [256]byte{
	,
}

// cssTab3 is a 512-entry table indexed by the 9-bit LFSR1 low state.
// Values repeat the pattern {0x00,0x24,0x49,0x6d,0x92,0xb6,0xdb,0xff} × 64.
var cssTab3 = func() [512]byte {
	var t [512]byte
	pat := [8]byte{0x00, 0x24, 0x49, 0x6d, 0x92, 0xb6, 0xdb, 0xff}
	for i := range t {
		t[i] = pat[i%8]
	}
	return t
}()

// cssTab4 is a bit-reversal table (256 entries).
var cssTab4 = [256]byte{
	,
}

// cssTab5 is the inverse bit-reversal table (256 entries).
var cssTab5 = [256]byte{
	,
}
