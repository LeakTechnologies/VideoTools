...
- **Cycle:** `v0.1.1-dev62` — **current**. dev61 (published 2026-09-03) shipped the rip density refinement (two-column 55/45 workspace, `ui.SectionBox` across all 8 module helpers) + the in-app updater asset-suffix fix; tester screenshots then found three structural layout bugs, so dev62 ships the **rip layout correction pass**: (1) **action bar rebuilt as a Border** — root cause was the readiness label's `TextWrapWord` inside an HBox (Fyne HBox children stretch to the container height; a wrapping label collapses to its narrowest word-boundary width then re-measures tall) which turned the bar into a giant band, stretched the PillButtons (they fill any given size), clipped the right column behind it, and rendered the label one-char-per-line; buttons now sit in the Border right edge at natural height, label is the centre with ellipsis truncation; (2) **compact empty states** — DiscSummary hides tech/main rows when empty, MenuPreview bounded 40px empty strip, ContentBrowser centred load-a-disc hint overlay; (3) dev61→dev62 is the **first in-app updater exercise** (dev61 was the tester's last manual download; dev60-and-older binaries still carry the suffix bug). Updater follow-ups (see TODO): release builds don't bake `buildCommit` via ldflags (same-tag patch detection dead), Linux has no tar.gz extraction path, stale "nightly PATCH" comment in `fetchUpdateInfo`. Earlier history: dev59/dev60 = rip view opening crash fixed (`updateDiscInfo` nil-closure on first render), Codeberg mirror retired, CI FFmpeg build resilience (restore-keys ×5 + download retries), MSIX `PKG_CONFIG_PATH` fix. **Windows FFmpeg builds are GREEN** (verified `69ec8610`).
...
| 2 | Tester verification of dev62 release | Layout corrections: compact full-width action bar (buttons never stretch vertically, readiness line single-line), compact empty states (DiscSummary title+hint only, MenuPreview strip, ContentBrowser hint), right column Format/Output/Status fully visible at ~1600×900 and 1280×720; plus the dev61→dev62 in-app update from a dev61 binary (first exercise of the asset-suffix fix) and all dev61/dev60 content; move roadmap cards `rip-refinement`/`rip-overhaul` `done` → `shipped` on sign-off |
