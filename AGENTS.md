...
- Current cycle: `v0.1.1-dev50` — **Media Engine gap closure complete. All 11 Phase 1 items shipped. HW decode default-on + error concealment shipped. Playlist, HDR tone-mapping, UDF reader fixes, collapsible player panel (Convert), logging improvements, CI DLL cache hardening, and updater sidecar refresh all shipped.** Priorities below.
...
| **Collapsible player panel in Convert** — `BuildCollapsibleHeader(t.ConvertSectionPlayer, ...)` wraps `videoPanel`; toggle sets `leftColumn.SetOffset(0.5/0.03)`; `ConvertSectionPlayer` i18n key all 4 locales | `main.go`, `internal/i18n/strings.go` + locale files | **SHIPPED** |
