...
- **Cycle:** `v0.1.1-dev60` — **current**. dev58 shipped + released the rip module overhaul (UX). dev59 was tagged (`a188ed7c`) with the rip crash fix, Codeberg mirror retirement, blocked-straggler cleanup and CI FFmpeg build resilience, but its GitHub Release was **never published**: the windows FFmpeg cold build red'd on a transient source-download flake, the `release` job (`needs: [linux, windows]`) got skipped, and "Re-run failed jobs" does not re-run skipped jobs. That content now ships as **dev60**, cut from the hardened `07a10c03`, so the tag pipeline runs the resilient workflow. dev59 = (1) the **rip view opening crash** fixed — `internal/app/modules/rip/view.go` assigned `updateDiscInfo` AFTER the initial `rebuildEnrich()` call, which invokes it, so the first render panicked on a nil closure, escaping `setContent`'s recover (the `buildRipView()` argument is evaluated before the recover arms) → silent process death with no `crashes.log` entry. Fixed by assigning `discSummary`/`updateDiscInfo` before the first `rebuildEnrich()`; `showRipView` also recovered+logs a stack trace to `crashes.log` before re-panicking; (2) **Codeberg mirror retired** (Codeberg will not host majority machine-generated code; Git recorded only GitHub remote); (3) **CI FFmpeg build resilience** — all five cache steps (dev/release × linux/windows, msix) gained `restore-keys` so tag runs reuse the previous tag's cached FFmpeg build, and all source downloads retry (curl `--retry 3 --retry-delay 5 --retry-all-errors` / msix wget `--tries=5 --retry-connrefused --waitretry=5`); (4) blocked-straggler cleanup (`scripts/windows/dev-verify.ps1`, legacy upscale dual-player render API removed, stale blocker docs retired). **Windows FFmpeg builds are GREEN** (verified `69ec8610`). A rip-density refinement then landed on top: two-column 55/45 workspace (CONTENT | PROCESSING), new `ui.SectionBox` shared component (compact 28px teal header, navy body, optional header actions) now backing all 8 module helpers (rip + audio/upscale/thumbnail/inspect/compare/filters/trim + trim stub), OUTPUT demoted to a plain bold subsection label, thumbnails 80×60→56×42, menu preview 320×180→280×158, DiscSummary empty-state spacer removed, `sectionGap` 10→6px. Build + vet + gofmt green across all 9 commits.
...
| 2 | Tester verification of dev60 rip density refinement | Two-column 55/45 workspace (CONTENT | PROCESSING) + ui.SectionBox migration (rip + 7 other modules), OUTPUT as plain subsection, thumbnails/menu preview shrunk, DiscSummary no longer stretches to fill; build+vet green — confirm the two-column reads clean at ~1600×900 and 1280×720 |
| 3 | Tester verification of dev60 release | Assets published (dev59 content cut as dev60 — dev59's release was never published); confirm the rip module opens without crash + disc-info populates on load; move roadmap cards `done` → `shipped` on sign-off |
| 4 | Tester verification of dvdvideo rip + concat fallback (dev55–dev57) | CI now builds libdvdread/libdvdnav + ships the dvdvideo demuxer, runtime probe no longer forces VOB concat, and dvdvideo now falls back to concat when libdvdnav rejects a source. Re-run a no-scan multi-VOB rip and confirm no crash at the VOB boundary |
