...
| 2 | Tester verification of dev60 release | Assets published (dev59 content cut as dev60 — dev59's release was never published); confirm the rip module opens without crash + disc-info populates on load; move roadmap cards `done` → `shipped` on sign-off |
