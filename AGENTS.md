...
- **Cycle:** `v0.1.1-dev58` — **released** (assets published, CI green). dev57 shipped the dvdvideo detection-string fixes (CI verify grep + `SupportsDVDVideo()` runtime probe), the dvdvideo→VOB concat runtime fallback (libdvdnav rejects some sources — nonzero reserved root-IFO field / no CSS auth — concat retries) and the CI dvdread.pc fail-fast guard. **Windows FFmpeg builds are GREEN** (verified `69ec8610`). dev58 ships the rip module overhaul (UX): `ScanDisc.VideoStandard` populated correctly (was read before the VTS cache filled, so NTSC/PAL never surfaced), disc info decoupled into `updateDiscInfo()`, a dedicated `DiscSummary` card with explicit empty/scanning/scanned/error states, a linear SOURCE→DISC→TITLES→OUTPUT→ACTION workflow (Content Browser reused as the TITLES section instead of dominating the screen), Advanced accordion for uncommon options (menus, region conversion, full-disc), a "Ready to rip N title(s)" readiness line, compact collapsible log, and a full i18n pass over the module's hardcoded labels/dialogs/region strings (en/fr/iu/iu_latin).
