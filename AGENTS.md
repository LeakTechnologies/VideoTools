...
- Current cycle: `v0.1.1-dev50` — **Media Engine gap closure complete. All 11 Phase 1 items shipped. HW decode default-on + error concealment shipped. Playlist, HDR tone-mapping, UDF reader fixes, collapsible player panel (Convert + Filters + Upscale), logging improvements, CI DLL cache hardening, and updater sidecar refresh all shipped.** Priorities below.
...
| **Collapsible player panel in Convert, Filters, Upscale** — `BuildCollapsibleHeader(t.ConvertSectionPlayer, ...)` wraps video area; `resolveOffset()` tracks `playerOpen`+`metaOpen`; matrix: both open=0.65/0.60, player closed=0.03, meta closed=0.97 | `main.go`, `internal/app/modules/filters/view.go`, `internal/app/modules/upscale/view.go` | **SHIPPED** |
