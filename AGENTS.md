...
- **Cycle:** `v0.1.1-dev71` — **current**. **dev71 = VOB-concat stale-PTS cap hardening + title-driven output names**: (1) the dev69 cap never engaged on a second grey-market disc (`11 (2000)`, same VOB layout as the dev69 title) — the stale PTS offset bakes into the *entire* second VOB, so it probes to +93822 s and the bundled static ffprobe errors on it, hitting the old "could not probe all VOB durations" branch → rip still 26h (and seeking into that inflated region crashes MPV — the file's timestamps are structurally broken, not just mis-labelled). The `-t` cap source is now hierarchical: per-VOB media-duration sum when sane (sum < 3× the IFO PGC play time — dev69 behaviour preserved), else the IFO PGC `titleInfo.Duration` (immune to stale file PTS offsets), so a safe cap ALWAYS engages; verified on `11 (2000)` (`-t 1884` → 1822.816 s, the real ~30m23s; concat+genpts view is clean with continuous packets — no seek breakage). (2) **the rip Title field drives the output filename by default** — a set Title names the file (`test` → `test.mkv` in DVD_Rips) and the full-disc dir (`FullDiscOutputTitlePath`); blank Title / source-format changes fall back to the source name; hand-editing the output path stops auto-naming (`outputTouched`). **dev70 released 2026-09-12** (tag `v0.1.1-dev70`, `83155474`): **rip UI polish** — mode radio reworded ("Movie + extras (choose titles)" / "Main feature only", stacked vertically, still defaults to the latter); title-card info line on one localized "N chapters · N audio · N subs" line (no word-wrap split, no duplicated T##/duration; new singular/plural `RipTitleAudioOne/Many` + `RipTitleSubsOne/Many` keys across en/fr/iu/iu_latin, `RipTitleCardFmt` now just "%d chapters"); subtitle-language picker gains Select All / Deselect All buttons (existing `RipSelectAll`/`RipDeselectAll` keys reused, per-language `OnChanged` suppressed/restored so the selection never duplicates, persisted). **dev69 released 2026-09-12** (tag `v0.1.1-dev69`, `e4e94407`): VOB-concat stale-PTS duration cap — grey-market discs whose VOBs carry a phantom tail packet at a huge PTS offset inflated fallback rips to "26hrs"; the executor probes per-VOB durations (`probeDuration`) and caps the output with `-t ceil(Σ + 60s)` via new `RipArgs.MaxDuration`, dropping those phantom-tail packets; verified end-to-end (uncapped reproduces 95443.815 s; capped yields 1822.784 s, the real 30m23s title) and on the lying-IFO disc (the dev68 clamped 9-subs run still completes with the cap applied, nothing truncated). **dev68 released 2026-09-12** (tag `v0.1.1-dev68`, `bf1e6522`): VOB-concat subtitle-map clamp for liar-IFO discs — some discs' IFOs advertise more subtitle languages than the VOBs carry physical subpicture streams for (grey-market media); the per-stream `-map 0:s:<idx>` flags from dev64 then reference streams that don't exist and ffmpeg hard-fails the whole rip (`Stream map '0:s:N' matches no streams`); on the dvdvideo→VOB-concat fallback the executor now probes the concat input's actual subtitle stream count (`probeSubtitleCount`) and clamps `SubtitleLangs`/`SubtitleSel` to the leading languages that exist, logging the drop — verified end-to-end on a real 10-IFO-language/9-stream disc. **dev67 released 2026-09-10** (tag `v0.1.1-dev67`, `fc444a78`): tester-requested rip de-clutter + settings keyboard-nav fix — (1) **rip default mode** — the mode radio lists first and defaults to "Full movie (main feature)" (was "Selected scenes"); (2) **rip de-clutter** — the Disc Menu preview panel and the in-app Rip Log strip are removed outright so the Content Browser owns the full left-column height (rip activity stays in the on-disk executor log; `menu_preview.go` deleted; obsolete `RipLog*`/`RipMenuPreview`/`RipPreserveMenus`/`RipLoadingMenu`/`RipNoMenuPlaceholder` i18n keys retired across en/fr/iu/iu_latin); (3) **settings keyboard-nav fix** — the dev66 `desktop.CustomShortcut` shortcuts were dead: the Fyne GLFW driver only synthesizes `CustomShortcut` when `modifier != 0` (`internal/driver/glfw/window.go`), so unmodified PageUp/PageDown/Home/End never fired from `canvas.AddShortcut`; rewritten to dispatch through a focused 1×1 key-catcher widget (`settings.Options.KeyCatcher`, `fyne.Focusable` + `desktop.Keyable`) whose `TypedKey` serves every plain keypress including repeats, with canvas `desktop.Canvas` `SetOnKeyDown`/`SetOnKeyUp` as the fallback when nothing is focused, previous handlers preserved, keys unregistered on leaving Settings). **dev66 released 2026-09-07** (tag `v0.1.1-dev66`, `ee73323d`): settings keyboard navigation + updater error-message hardening — PageUp/PageDown scroll the active Settings tab by one viewport, Home/End to top/bottom (`settings.Options.ActiveScroll` resolver on the visible tab via `tabs.OnSelected`), shortcuts registered only while Settings is on screen and removed on leave (wrapped `OnBack`); `describeUpdateError` surfaces connection/rate-limit/not-yet-published causes in the update-check failure dialog instead of a raw network detail, raw error logged under `CatSystem`, stale "nightly run" comment corrected. **dev65 released 2026-09-07** (tag `v0.1.1-dev65`, `acc2bc98`): rip refinement follow-up — vertical-space layout pass (Rip Log demoted to a collapsed-by-default strip, 0.97→0.72 bounded expansion, auto-open via `SetRipLogExpand`; Disc Menu preview collapses to ~40px until a menu frame loads; Content Browser regains the flexible height) + Load Disc from an optical drive (`OnLoadDisc` → `detectOpticalDrives` + radio drive picker → `resolveOpticalDriveVIDEOTS`, Windows drive letter / Linux `/proc/mounts`; `docs/RIP_LOAD_DISC.md`). dev62 (published 2026-09-03T14:32:27Z, tag `v0.1.1-dev62`, `ccb4f213`) shipped the **rip layout correction pass** (action bar rebuilt as a Border — root cause was the readiness label's `TextWrapWord` inside an HBox which created a giant button band, stretched the PillButtons, clipped the right column, and rendered the label one-char-per-line; plus compact empty states: DiscSummary hides tech/main rows, MenuPreview bounded 40px strip, ContentBrowser centred hint). dev62 was the **first in-app updater exercise** (dev61→dev62). All dev62 pipelines green (dev CI `33765093161`, release `33765516968`, MSIX `33765516935`). **dev63 = base cycle + dev62 follow-up patches** the tester reported: (1) **Convert metadata-panel fold bug** — folding the panel hid its tappable `METADATA` header too (the header lived inside the hidden `metaPanel`), so it could never be re-expanded; now the header stays visible and only the body hides, applied to Convert + Filters + Upscale (`buildMetadataPanel` gained `initiallyOpen`); (2) **rip ISO-load scanning-state bug** — `SetScanning()` ran before `rebuildEnrich()` clobbered it with `SetEmpty()`, so loading an ISO never showed "Reading disc information"; `SetScanning()` now runs after the enrich rebuild, `runISOScan` converts any UDF-reader panic into a visible `SetError` (was a process kill / stuck empty card) with CatDVD start/done logs, and re-entering the rip module re-scans a previously chosen source. **dev63 released 2026-09-04** (tag `v0.1.1-dev63`, `87a0c6ad`). **dev64 = rip mode selection (scenes / full movie of the longest title), per-language subtitle selection (one checkbox per main-title language → per-stream `-map 0:s:<idx>`, VOBSUB pairs kept together, legacy configs default to all; `RipArgs.SubtitleSel []int`), scan-as-you-go disc snippets in the DiscSummary (`onNote` → `SetSnippet`), and the Convert SMPTE-idle restore** (`InlineVideoPlayer.CurrentPath()` + `Close()` clearing the path; `showConvertView` resets the shared primary player on menu-driven re-entry unless it already shows the convert source). Updater follow-ups (see TODO): release builds don't bake `buildCommit` via ldflags (same-tag patch detection dead), Linux has no tar.gz extraction path, stale "nightly PATCH" comment in `fetchUpdateInfo`. Earlier history: dev59/dev60 = rip view opening crash fixed (`updateDiscInfo` nil-closure on first render), Codeberg mirror retired, CI FFmpeg build resilience (restore-keys ×5 + download retries), MSIX `PKG_CONFIG_PATH` fix. **Windows FFmpeg builds are GREEN** (verified `69ec8610`).
