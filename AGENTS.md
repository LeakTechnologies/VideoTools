...
| 2 | Tester verification of dev66 + dev65 + dev64 releases | **dev66**: settings keyboard nav (PageUp/PageDown page the active tab by one viewport, Home/End to top/bottom, keys stop after leaving Settings) + the update-check failure dialog explains the cause (rate-limit / not-yet-published / no network). **dev65**: rip refinement — at ~1600×900 the Content Browser shows 4+ titles without scrolling, Rip Log is just a LOG pill (bounded ~28% expansion, auto-open on rip/error), Disc Menu preview is a ~40px strip until a menu frame loads (Clear ISO resets it); LOAD DISC loads a physical DVD (drive picker when several drives; data disc/empty/no drive → clean visible error). **dev64**: rip mode radio (scenes / full movie of the longest title), per-language subtitle checkboxes (deselect a language → streams stripped; legacy configs default to all), scan-as-you-go DiscSummary snippets, Convert SMPTE idle restored on menu-driven re-entry + the dev63/dev62 patches (metadata fold keeps the tappable header visible; ISO load shows "Reading disc information"; dev62 layout corrections) + the dev61→dev62 in-app update from a dev61 binary; move roadmap cards `rip-refinement`/`rip-overhaul` `done` → `shipped` on sign-off |
