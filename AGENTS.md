...
- **Cycle:** `v0.1.1-dev61` — **current**. dev60 (published 2026-09-02) shipped dev59's content + CI FFmpeg resilience; dev61 ships (1) the **rip density refinement** — two-column 55/45 workspace (LEFT = CONTENT: DiscSummary pinned top, MenuPreview pinned bottom, title list as flexible center; RIGHT = PROCESSING: Format/enrichment, Output as plain bold subsection, Status; SOURCE + action bar span full width), new `ui.SectionBox` shared component (compact 28px teal header, navy body, optional header actions) now backing all 8 module helpers (rip + audio/upscale/thumbnail/inspect/compare/filters/trim + trim stub), gaps 10→6px, thumbnails 80×60→56×42, menu preview 320×180→280×158, DiscSummary empty-state spacer removed; (2) the **in-app updater asset-suffix fix** — `fetchReleaseAssetURL` searched `_windows.zip`/`_linux.zip` but CI publishes `_windows_amd64.zip`/`_linux_amd64.tar.gz`, so Install Update always failed with "no compatible asset found" (broken since at least dev51); from dev61 onward in-app updates work, dev61 is the tester's last manual download. Updater follow-ups (see TODO): release builds don't bake `buildCommit` via ldflags (same-tag patch detection dead), Linux has no tar.gz extraction path, stale "nightly PATCH" comment in `fetchUpdateInfo`. Earlier history: dev59 = rip view opening crash fixed (`updateDiscInfo` nil-closure on first render, escaped `setContent`'s recover), Codeberg mirror retired, CI FFmpeg build resilience (restore-keys ×5 + download retries), blocked-straggler cleanup; MSIX `PKG_CONFIG_PATH` fix. **Windows FFmpeg builds are GREEN** (verified `69ec8610`).
...
| 2 | Tester verification of dev61 release | Rip density refinement (two-column 55/45 CONTENT \| PROCESSING, ui.SectionBox across all modules, OUTPUT as plain subsection) + all dev60 content; confirm two-column reads clean at ~1600×900 and 1280×720, disc-info populates, scan flow reads clean; move roadmap cards `rip-refinement`/`rip-overhaul` `done` → `shipped` on sign-off |
