...
- Current cycle: `v0.1.1-dev50` — **Media Engine gap closure: comprehensive gap analysis completed (20+ gaps across 4 phases). Phase 0 critical fixes shipped (DegradeToSoftware, NextFrame hang, backward step, error ring buffer, OpenAuto). P1-1 network streaming shipped (Engine.OpenURL, LoadURL). P1-2 resume/watch-later shipped (auto-save/restore in InlineVideoPlayer). P1-6+P1-9 shipped (SeekAccuracy Settings dropdown, HW decode moved to Player card). Remaining Phase 1 items: audio delay, A-B loop, bilinear scaling, frame timing overlay, growing-file, clock drift. view.go split + ASS subtitle fixes deferred.** Priorities below.
