...
- [ ] **Convert module: player-panel SMPTE bars + UI audit** — tester report: the SMPTE colour bars don't show in the Convert module's player panel when idle. Likely causes to check: `GetConvertPlayer()` returns the shared `primaryInlinePlayer` singleton (`convert_player_native.go`), so re-entering Convert can surface a stale frame from another module instead of the idle bars — Convert never calls `player.Close()`/`SetFrame(nil)` on re-entry (unlike rip's `clearISOBtn`); also audit the convert stage min-size/overlay (transport bar, drop zone) and `buildVideoPaneNative` layout while in there. Deferred until the rip dev63 batch lands.
