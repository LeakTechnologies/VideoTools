...
- [ ] **Convert module: player-panel SMPTE bars + UI audit** — core fix landed (`1863bc0f`): `showConvertView` resets the shared primary player to idle (via `Close`) on menu-driven re-entry unless it's already showing Convert's own source; `InlineVideoPlayer.CurrentPath()` added, `Close()` clears the path. Remaining audit sub-scope: convert stage min-size/overlay (transport bar disappears on nil-source builds, drop zone bounds, aspect sizing) and `buildVideoPaneNative` layout — verify with tester that SMPTE idle now returns after visiting another playback module (e.g. Inspect → menu → Convert).
