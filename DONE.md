...
- **Rip density refinement + global header migration** — dev58's linear vertical flow (SOURCE→DISC→TITLES→OUTPUT→ACTION as one long scroll) replaced with a two-column workspace. New `ui.SectionBox` shared component (compact 28px teal header, navy body, optional header actions) replaces the per-module `buildXxxBox` duplication (rip: view.go/disc_summary.go + 7 other module funds: audio/upscale/thumbnail/inspect/compare/filters/trim + trim stub). Left column = CONTENT (DiscSummary pinned top, MenuPreview pinned bottom, title list as the flexible center with internal scroll); right column = PROCESSING (Format/enrichment, Output as a plain bold subsection, Status), each scrollable. Action bar spans the full width at the bottom (readiness line left, Add to Queue / Open in Player / RIP NOW right). Density: section gaps 10px→6px, thumbnails 80×60→56×42, menu preview 320×180→280×158, DiscSummary empty-state spacer removed so the card keeps natural height. Build + vet + gofmt green on all 9 commits.
